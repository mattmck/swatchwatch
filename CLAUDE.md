...
- `AZURE_OPENAI_BATCH_ENABLED` — set to a truthy value (`true`/`1`/`yes`/`on`) to use Azure OpenAI Batch API for Shopify ingestion jobs with ≥ 5 image candidates; read at call time so no restart needed after toggling; defaults to `false`
