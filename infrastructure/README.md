...
| API Management *(optional)* | `azurerm_api_management.main` | APIM scaffold (Consumption): provisions `azurerm_api_management.main` and pre-wires Function App gateway settings when `apim_enabled=true`; Azure OpenAI traffic is not routed through APIM until API/backends/policies are added and `openai_gateway_enabled=true`. |
...
| `apim_enabled` | `true` | Provision APIM scaffold resources (`azurerm_api_management.main`) and pre-wire Function App gateway settings. This alone does not route Azure OpenAI traffic. |
...
| `openai_gateway_enabled` | `false` | Function App setting `AZURE_OPENAI_USE_GATEWAY`; set to `true` only after APIM API/backends/policies are implemented and validated. |
...
| `apim_name` | APIM service name for the scaffolded APIM instance (empty when `apim_enabled=false`) |
| `apim_gateway_url` | APIM gateway URL for the scaffolded APIM instance (empty when `apim_enabled=false`) |
| `openai_gateway_endpoint` | APIM OpenAI gateway base URL exposed to Functions via `AZURE_OPENAI_GATEWAY_ENDPOINT`; active traffic routing requires follow-up APIM API/backends/policies plus `openai_gateway_enabled=true`. |
...
- When `apim_enabled=true`, Terraform provisions APIM scaffold infrastructure (Consumption by default) and pre-wires Function App gateway settings:
  - `AZURE_OPENAI_GATEWAY_ENDPOINT`
  - `AZURE_OPENAI_GATEWAY_SUBSCRIPTION_KEY` (if `apim_openai_subscription_key` is provided)
  - `AZURE_OPENAI_USE_GATEWAY` (controlled by `openai_gateway_enabled`)
  Routing through APIM does not start until the follow-up APIM API/backends/policies are implemented and `openai_gateway_enabled=true`.
